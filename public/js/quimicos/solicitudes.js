  ...
  try {
    ...
    if (data.success) {
      ...
      if (action == "approve") {
        notification(
          "error",
          "Falló al aprobar la solicitud, intenta nuevamente.",
          2000,
        );
      } else {
        notification(
          "error",
          "Falló al rechazar la solicitud, intenta nuevamente.",
          2000,
        );
      }
    }
...
document
  ...
  .addEventListener("click", async function () {
    ...
    try {
      const responseCadencias = await fetch(
        "../../Handler/quimicos/solicitudesHandler.php?action=obtenerCadencias",
        {
          method: "GET",
        },
      );
      const cadencias = await responseCadencias.json();
      const cadenciasEncoded = encodeURIComponent(JSON.stringify(cadencias));
      ...
      const cadenciaActual = await responseCadenciaActual.json();
      const cadenciaActualEncoded = encodeURIComponent(
        JSON.stringify(cadenciaActual),
      );

      var url = `_editCadencia.php?cadencias=${cadenciasEncoded}&cadenciaActual=${cadenciaActualEncoded}`;
