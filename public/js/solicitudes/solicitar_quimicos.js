...
$(document).ready(function () {
  document
    .getElementById("cedula_solicitante")
  ...
  $("#id_quimico_solicitud_consumo").on("change", function () {
    ...
    const boton = $("#btnTarjetaEmergencia");

    if (
      quimicoSeleccionado &&
      quimicoSeleccionado.url_etiqueta_emergencia_quimico &&
      quimicoSeleccionado.url_etiqueta_emergencia_quimico.trim() !== ""
    ) {
      boton
        .attr("href", quimicoSeleccionado.url_etiqueta_emergencia_quimico)
        .removeClass("d-none");
    } else {
      boton.attr("href", "#").addClass("d-none");
    }
  });

  document
    .getElementById("cantidad_solicitud_consumo")
    .addEventListener("input", function () {
  ...
  $("#cantidad_solicitud_consumo").on("input", function () {
    ...
    let maxRetiro = parseFloat(
      quimicoSeleccionado.cantidad_maxima_retiro_quimico,
    );
    let disponible = parseFloat(
      quimicoSeleccionado.cantidad_disponible_quimico,
    );
    ...
    if (cantidad > maxRetiro) {
      notification(
        "error",
        `La cantidad máxima permitida es de ${maxRetiro} ${umb}.`,
        2000,
      );
      $(this).val(maxRetiro);
      return;
  ...
  document
    .getElementById("formSolicitudQuimico")
    .addEventListener("submit", async function (e) {
      ...
      try {
        const response = await fetch(
          "../../Handler/solicitudes/solicitarQuimicosHandler.php",
          {
            method: "POST",
        ...
        if (resultado.success) {
          notification(
            "success",
            "El químico se solicitó correctamente, espera la respuesta.",
            3000,
          );
